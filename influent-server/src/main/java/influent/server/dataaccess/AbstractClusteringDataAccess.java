...
public abstract class AbstractClusteringDataAccess implements FL_ClusteringDataAccess {
  ...
  protected static final Logger s_logger =
      LoggerFactory.getLogger(AbstractClusteringDataAccess.class);
  ...
  public List<FL_Cluster> getAccountOwners(List<String> ownerIds) {
    ...
    for (FL_Entity owner : ownerEntities) {
      PropertyHelper summary =
          EntityHelper.getFirstPropertyByTag(owner, FL_PropertyTag.CLUSTER_SUMMARY);
      if (summary != null) {
    ...
    Map<String, List<FL_Entity>> accountsForAccountOwners =
        _entityAccess.getAccounts(new ArrayList<String>(accountOwners.keySet()));
    ...
    for (String id : accountOwners.keySet()) {
      ...
      FL_Cluster ownerCluster =
          _clusterFactory.toAccountOwnerSummary(owner, accounts, new ArrayList<FL_Cluster>(0));
  ...
  private void addOwnerProperties(FL_Cluster ownerCluster) {
    ...
    if (prop != null) {
      ...
      List<FL_Entity> ownerEntity =
          _entityAccess.getEntities(Collections.singletonList(ownerId), FL_LevelOfDetail.FULL);
  ...
  private void addOwnerProperties(FL_Cluster ownerCluster, FL_Entity ownerEntity) {
    List<FL_PropertyTag> filter =
        Arrays.asList(
            FL_PropertyTag.ENTITY_TYPE,
  ...
  public List<FL_Cluster> getClusterSummary(List<String> clusterIds) {
    ...
    try {
      ...
      for (Map.Entry<String, List<String>> entry : entitiesByType.entrySet()) {
        ...
        Map<String, Map<String, PropertyHelper>> entityPropMap =
            new HashMap<String, Map<String, PropertyHelper>>();

        String summaryTable =
            _applicationConfiguration.getTable(
                entityType, CLUSTER_SUMMARY.name(), CLUSTER_SUMMARY.name());
        String summaryPropertyColumn =
            _applicationConfiguration.getColumn(
                entityType, CLUSTER_SUMMARY.name(), CLUSTER_SUMMARY_PROPERTY.name());
        String summaryTagColumn =
            _applicationConfiguration.getColumn(
                entityType, CLUSTER_SUMMARY.name(), CLUSTER_SUMMARY_TAG.name());
        String summaryTypeColumn =
            _applicationConfiguration.getColumn(
                entityType, CLUSTER_SUMMARY.name(), CLUSTER_SUMMARY_TYPE.name());
        String summaryValueColumn =
            _applicationConfiguration.getColumn(
                entityType, CLUSTER_SUMMARY.name(), CLUSTER_SUMMARY_VALUE.name());
        String summaryStatColumn =
            _applicationConfiguration.getColumn(
                entityType, CLUSTER_SUMMARY.name(), CLUSTER_SUMMARY_STAT.name());

        // process src nodes in batches
        List<String> idsCopy =
            new ArrayList<String>(
                entitySubgroup); // copy the ids as we will take 1000 at a time to process and the
        // take method is destructive
        while (idsCopy.size() > 0) {
          List<String> tempSubList =
              (idsCopy.size() > 1000) ? idsCopy.subList(0, 999) : idsCopy; // get the next 1000
          List<String> subIds =
              new ArrayList<String>(tempSubList); // copy as the next step is destructive
          tempSubList
              .clear(); // this clears the IDs from idsCopy as tempSubList is backed by idsCopy

          String finEntityTable =
              _applicationConfiguration.getTable(entityType, FIN_ENTITY.name(), FIN_ENTITY.name());
          String finEntityEntityIdColumn =
              _applicationConfiguration.getColumn(entityType, FIN_ENTITY.name(), ENTITY_ID.name());
          ApplicationConfiguration.SystemColumnType finEntityEntityIdColumnType =
              _applicationConfiguration.getColumnType(
                  entityType, FIN_ENTITY.name(), ENTITY_ID.name());
          String finEntityUniqueInboundDegree =
              _applicationConfiguration.getColumn(
                  entityType, FIN_ENTITY.name(), UNIQUE_INBOUND_DEGREE.name());
          String finEntityUniqueOutboundDegree =
              _applicationConfiguration.getColumn(
                  entityType, FIN_ENTITY.name(), UNIQUE_OUTBOUND_DEGREE.name());

          Map<String, int[]> entityStats = new HashMap<String, int[]>();

          // Create prepared statement
          String preparedStatementString =
              buildPreparedStatementForInboundOutboundDegree(
                  subIds.size(),
          ...
          for (int i = 0; i < subIds.size(); i++) {
            stmt.setString(
                index++, getNamespaceHandler().toSQLId(subIds.get(i), finEntityEntityIdColumnType));
          }
          ...
          preparedStatementString =
              buildPreparedStatementForClusterSummary(
                  subIds.size(),
                  finEntityEntityIdColumn,
          ...
          for (String id : entityPropMap.keySet()) {
            ...
            if (stats != null) {
              ...
              propMap.put(
                  "inboundDegree",
                  new PropertyHelper("inboundDegree", stats[0], FL_PropertyTag.INFLOWING));
              propMap.put(
                  "outboundDegree",
                  new PropertyHelper("outboundDegree", stats[1], FL_PropertyTag.OUTFLOWING));
            }
          }
    ...
    try {
      ...
      for (Map.Entry<String, List<String>> entry : entitiesByType.entrySet()) {
        ...
        String summaryMemberTable =
            _applicationConfiguration.getTable(
                entityType, CLUSTER_SUMMARY_MEMBERS.name(), CLUSTER_SUMMARY_MEMBERS.name());
        String summaryIdColumn =
            _applicationConfiguration.getColumn(
                entityType, CLUSTER_SUMMARY_MEMBERS.name(), CLUSTER_SUMMARY_SUMMARY_ID.name());
        String summaryMemberIdColumn =
            _applicationConfiguration.getColumn(
                entityType, CLUSTER_SUMMARY_MEMBERS.name(), CLUSTER_SUMMARY_ENTITY_ID.name());
        ...
        while (idsCopy.size() > 0) {
          ...
          // Create prepared statement
          String preparedStatementString =
              buildPreparedStatementForClusterSummaryMembers(
                  subIds.size(), summaryIdColumn, summaryMemberIdColumn, summaryMemberTable);
          PreparedStatement stmt = connection.prepareStatement(preparedStatementString);
          ...
          for (int i = 0; i < subIds.size(); i++) {
            stmt.setString(
                index++,
                getNamespaceHandler()
                    .toSQLId(subIds.get(i), ApplicationConfiguration.SystemColumnType.STRING));
          }
  ...
  protected PropertyHelper createProperty(
      String property, String tag, String type, String value, float stat) {
    ...
    if (isDist) {
      ...
      FL_DistributionRange range =
          FL_DistributionRange.newBuilder()
              ...
              .setRangeType(FL_RangeType.DISTRIBUTION)
              .setType(propType)
              .setIsProbability(false)
              .build();
      prop =
          new PropertyHelper(
              property, friendlyText, null, null, Collections.singletonList(propTag), range);
    } else {
  ...
  protected void updateProperty(
      PropertyHelper property, String tag, String type, String value, float stat) {
    ...
    // update distribution range - assumes updateProperty is only applied to distribution properties
  ...
  protected List<FL_Cluster> createSummaryClusters(
    ...
    for (String id : entityPropMap.keySet()) {
      ...
      summaries.add(
          new ClusterHelper(
              getNamespaceHandler()
                  .globalFromLocalEntityId(InfluentId.CLUSTER_SUMMARY, entityType, id),
              label,
              tagList,
              props,
  ...
  private String buildPreparedStatementForClusterSummaryMembers(
      int numIds, String summaryIdColumn, String summaryMemberIdColumn, String summaryMemberTable) {
    if (numIds < 1
