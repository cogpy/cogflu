...
public class SearchSQLHelper {
  ...
  public SQLFilter buildSpecialTermFilter(FL_PropertyMatchDescriptor term, String type) {
    ...
    if (term.getKey().equals(FL_RequiredPropertyKey.ENTITY.name())) {
      FL_PropertyMatchDescriptor fromTerm =
              ...
              .setTypeMappings(
                  Collections.singletonList(
                      FL_TypeMapping.newBuilder()
                          .setType(type)
                          .setMemberKey(_mapKey(FL_RequiredPropertyKey.FROM.name(), type))
                          .build()))
              .build();
          ...
          FL_PropertyMatchDescriptor.newBuilder(term)
              ...
              .setTypeMappings(
                  Collections.singletonList(
                      FL_TypeMapping.newBuilder()
                          .setType(type)
                          .setMemberKey(_mapKey(FL_RequiredPropertyKey.TO.name(), type))
                          .build()))
              .build();
      ...
      FL_PropertyMatchDescriptor transIdTerm =
          FL_PropertyMatchDescriptor.newBuilder(term)
              ...
              .setTypeMappings(
                  Collections.singletonList(
                      FL_TypeMapping.newBuilder()
                          .setType(type)
                          .setMemberKey(FL_RequiredPropertyKey.LINKED.name())
                          .build()))
              .build();
  ...
  SQLFilterColumn _buildFilterColumn(FL_PropertyMatchDescriptor term, String type) {
    ...
    if (range instanceof FL_SingletonRange) {
      ...
      throw new UnsupportedOperationException(
          "Trying to create a filter for term "
              + term.getKey()
              + " of an unsupported range type "
              + range.getClass());
    }
  ...
  /**
   * General method to build a {@link SQLFilter} from a {@link FL_PropertyMatchDescriptor}. This can
   * handle {@link FL_RequiredPropertyKey}s and ID related fields along with normal fields.
   *
   * @param term
   * @return
   */
  ...
  public SQLFilter buildListOfFilters(
      Iterable<FL_PropertyMatchDescriptor> terms, String type, SQLFilterGroup group) {
  ...
  /**
   * Top level method that will pull out all optional and required terms separately so they can be
   * turned into separate AND/OR filters.
   *
   * @param terms
   * @return
   */
  ...
  private FL_Property propertyFromQueryResult(
      FL_PropertyDescriptor pd, List<Object> valueList, FL_LevelOfDetail levelOfDetail) {

    // Test to see if the property is set hidden, or if it's hidden by Level of Detail
    boolean isHidden =
        pd.getLevelOfDetail().equals(FL_LevelOfDetail.HIDDEN)
            || (levelOfDetail.equals(FL_LevelOfDetail.SUMMARY)
                && pd.getLevelOfDetail().equals(FL_LevelOfDetail.FULL));

    Object range;
    // Create a list range, or a singleton range depending on how many values we got
    if (valueList.size() > 1) {
      range = FL_ListRange.newBuilder().setType(pd.getPropertyType()).setValues(valueList).build();
    } else {
      range =
          FL_SingletonRange.newBuilder()
    ...
    return new PropertyHelper(
        pd.getKey(), pd.getFriendlyText(), null, null, pd.getTags(), isHidden, range);
  }
  ...
  public List<Object> getObjectsFromTerms(
    ...
    for (Map.Entry<String, List<FL_PropertyMatchDescriptor>> entry : termMap.entrySet()) {
              ...
              if (fields != null) {
                for (PropertyField field : fields) {
                  final String fieldKey =
                      PropertyDescriptorHelper.getFieldname(
                          field.getProperty(), entry.getKey(), null);
      ...
      List<List<Object>> queryResults =
          fetchColumnsForTerms(
              table, columns, entry.getValue(), entry.getKey(), orderBy, limitResults);
      ...
      for (List<Object> result : queryResults) {
        ...
        for (FL_PropertyDescriptor pd : composites) {
          ...
          if (pd.getPropertyType() == FL_PropertyType.GEO) {
            ...
            for (int i = 0; i < latValues.size(); i++) {
              ...
              FL_GeoData.Builder geoDataBuilder =
                  FL_GeoData.newBuilder()
            ...
            throw new UnsupportedOperationException(
                "Unhandled composite type: " + pd.getPropertyType().name());
          }
        }
        ...
        if (_helperType == FL_Entity.class) {
          ...
          results.add(
              new EntityHelper(
                  uid, type, Collections.singletonList(FL_EntityTag.ACCOUNT), null, null, props));
        } else {
  ...
  String getUIDFromProperties(
      List<FL_Property> propList, FL_RequiredPropertyKey key, Class<?> objectType) {
  ...
  private SQLSelect _createSubQuery(
      String type, String table, List<String> columns, List<FL_PropertyMatchDescriptor> terms) {
    ...
    groupQuery.from(table, "f").where(buildTermsFilter(terms, type));

    return groupQuery;
  }

  // Create a top level query containing subqueries
  private SQLSelect _createQuery(
      String type,
      String table,
      List<String> columns,
      List<List<FL_PropertyMatchDescriptor>> groupedTerms) {
  ...
  private boolean _buildSpecialIdPairs(
      Iterable<FL_PropertyMatchDescriptor> terms, String type, SQLFilterGroup group) {
    ...
    // TODO: Handle cases where multiple ENTITY or LINKED terms have been specified. Currently
    // throws an error
    ...
    if (entityIDTerm != null && linkedIDTerm != null) {
      FL_PropertyMatchDescriptor fromEntityIDTerm =
          FL_PropertyMatchDescriptor.newBuilder(entityIDTerm)
              .setKey(FL_RequiredPropertyKey.FROM.name())
              .setTypeMappings(
                  Collections.singletonList(
                      FL_TypeMapping.newBuilder()
                          ...
                          .build()))
              .build();
      FL_PropertyMatchDescriptor toLinkedIDTerm =
          FL_PropertyMatchDescriptor.newBuilder(linkedIDTerm)
              .setKey(FL_RequiredPropertyKey.TO.name())
              .setTypeMappings(
                  Collections.singletonList(
                      FL_TypeMapping.newBuilder()
      ...
      FL_PropertyMatchDescriptor fromLinkedIDTerm =
          FL_PropertyMatchDescriptor.newBuilder(linkedIDTerm)
              .setKey(FL_RequiredPropertyKey.FROM.name())
              .setTypeMappings(
                  Collections.singletonList(
                      FL_TypeMapping.newBuilder()
                          ...
                          .build()))
              .build();
      FL_PropertyMatchDescriptor toEntityIDTerm =
          FL_PropertyMatchDescriptor.newBuilder(entityIDTerm)
              .setKey(FL_RequiredPropertyKey.TO.name())
              .setTypeMappings(
                  Collections.singletonList(
      ...
      SQLFilter entityIDFilter =
          _sqlBuilder.and(
              _buildFilterColumn(fromEntityIDTerm, type), _buildFilterColumn(toLinkedIDTerm, type));
      SQLFilter linkedIDFilter =
          _sqlBuilder.and(
              _buildFilterColumn(fromLinkedIDTerm, type), _buildFilterColumn(toEntityIDTerm, type));
  ...
  private List<List<FL_PropertyMatchDescriptor>> _groupFilterTerms(
      List<FL_PropertyMatchDescriptor> filterTerms) {
    List<List<FL_PropertyMatchDescriptor>> groupedFilterTerms =
        new LinkedList<List<FL_PropertyMatchDescriptor>>();

    List<FL_PropertyMatchDescriptor> generalFilterTerms =
        new LinkedList<FL_PropertyMatchDescriptor>();

    // Map of id term lists by id term type
    Map<String, List<FL_PropertyMatchDescriptor>> idFilterTermsMap =
        new HashMap<String, List<FL_PropertyMatchDescriptor>>();
    ...
    if (largestIdField != null) {
      List<FL_PropertyMatchDescriptor> idFilterTerms =
          idFilterTermsMap.get(largestIdField); // entry.getValue();
        ...
        if (filterIds != null) {
          for (int j = 0; j < filterIds.size(); j++) {
            ...
            if (batchFilterIds.size() >= ID_BATCH_SIZE || lastItem) {
              // we have filled up the batch - create a property match descriptor for this batch of
              // filter ids
              FL_PropertyMatchDescriptor batchedPmd =
                  FL_PropertyMatchDescriptor.newBuilder(pmd)
                      .setRange(
                          FL_ListRange.newBuilder()
                              .setType(FL_PropertyType.STRING)
                              .setValues(batchFilterIds)
                              .build())
                      .build();
              ...
              if (!lastItem) {
                ...
                batch.addAll(generalFilterTerms);
                // Add unbatched id terms
                for (Map.Entry<String, List<FL_PropertyMatchDescriptor>> entry :
                    idFilterTermsMap.entrySet()) {
                  if (!entry.getKey().equals(largestIdField)) {
                    batch.addAll(entry.getValue());
                  }
                }
  ...
  private String formatColumnValue(FL_PropertyType type, String value) {
    if (type.equals(FL_PropertyType.DOUBLE)
        || type.equals(FL_PropertyType.FLOAT)
        || type.equals(FL_PropertyType.INTEGER)
        || type.equals(FL_PropertyType.LONG)) {
      return value;
    } else {
