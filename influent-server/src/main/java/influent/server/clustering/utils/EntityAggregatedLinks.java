...
public class EntityAggregatedLinks {
  ...
  public static Map<String, List<FL_Link>> getRelatedAggregatedLinks(
    ...
    try {
      ...
      List<String> srcSummaryIds =
          InfluentId.filterInfluentIds(entityIds, InfluentId.CLUSTER_SUMMARY);
      ...
      Map<String, String> srcEntityAncestorIndex =
          new HashMap<String, String>(); // entity id -> ancestor id
      Map<String, String> dstEntityAncestorIndex =
          new HashMap<String, String>(); // entity id -> ancestor id

      // Get all the atomic children ids associated with src things and create ancestor map
      List<String> srcChildIds =
          buildEntityAncestorMap(srcThings, srcEntityAncestorIndex, srcContext);

      // filter the source children by type
      List<String> srcChildEntityIds =
          InfluentId.filterInfluentIds(srcChildIds, InfluentId.ACCOUNT);
      List<String> srcChildOwnerIds =
          InfluentId.filterInfluentIds(srcChildIds, InfluentId.ACCOUNT_OWNER);
      List<String> srcChildSummaryIds =
          InfluentId.filterInfluentIds(srcChildIds, InfluentId.CLUSTER_SUMMARY);

      s_logger.info(
          "Created ancestor map (total time : "
              + (System.currentTimeMillis() - startms) / 1000
              + "s)");
      ...
      // Get the related links for all the child srcEntities and add to the raw related links for
      // further processing
      rawLinks.putAll(
          da.getFlowAggregation(
              srcChildEntityIds,
              new ArrayList<String>(0),
              direction,
              FL_LinkEntityTypeFilter.ACCOUNT,
              dateRange));
      ...
      rawLinks.putAll(
          da.getFlowAggregation(
              ...
              new ArrayList<String>(0),
              direction,
              FL_LinkEntityTypeFilter.CLUSTER_SUMMARY,
              dateRange));
      ...
      s_logger.info(
          "Found related raw links (total time : "
              + (System.currentTimeMillis() - startms) / 1000
              + "s)");
      ...
      List<String> dstClusterSummaryIds =
          InfluentId.filterInfluentIds(dstIds, InfluentId.CLUSTER_SUMMARY);
      ...
      // We now have loop free raw links from all src and dst immutable objects (entities, immutable
      // clusters)
      ...
      s_logger.info(
          "Found "
              + dstEntities.size()
              + " destination entities (total time : "
              + (System.currentTimeMillis() - startms) / 1000
              + "s)");
      ...
      clusterer.clusterEntities(
          dstEntities, dstImmutableClusters, srcContext.getUid(), dstContext.getUid());

      s_logger.info(
          "Done clustering link entities (total time : "
              + (System.currentTimeMillis() - startms) / 1000
              + "s)");
      ...
      s_logger.info(
          "Created link entity ancestor map (total time : "
              + (System.currentTimeMillis() - startms) / 1000
              + "s)");
      ...
      Map<String, List<FL_Link>> agLinks =
          aggregateLinks(rawLinks, direction, srcEntityAncestorIndex, dstEntityAncestorIndex);
      ...
      s_logger.info(
          "Aggregation complete (aggregation took "
              + (System.currentTimeMillis() - startms) / 1000
              + "s)");
  ...
  private static void filterLoopEntitiesAndLinks(
    ...
    for (String key :
        new HashSet<String>(
            rawLinks.keySet())) { // make copy of keyset to avoid concurrent modification exception
  ...
  private static void filterDuplicateLinks(
      ContextRead targetContext, Map<String, List<FL_Link>> rawLinks, ClusterContextCache cache) {
    ...
    try {
      ...
      if (targetContext != null) {
        ...
        for (String key :
            new HashSet<String>(
                rawLinks
                    .keySet())) { // make copy of keyset to avoid concurrent modification exception
          List<FL_Link> links = rawLinks.get(key);
          List<FL_Link> filteredLinks = new LinkedList<FL_Link>();
          for (FL_Link link : links) {
            String destId =
                link.getTarget().equalsIgnoreCase(key) ? link.getSource() : link.getTarget();
  ...
  public static Map<String, List<FL_Link>> getAggregatedLinks(
    ...
    try {
      s_logger.info(
          "Started getAggregatedLinks for "
              + srcEntityIds.size()
              + " src entities and "
              + dstEntityIds.size()
              + " dst entities");
      ...
      if (srcContext != null) {
        srcThings.addAll(
            srcContext.getClusters(InfluentId.filterInfluentIds(srcEntityIds, InfluentId.CLUSTER)));
      }
      ...
      if (dstContext != null) {
        dstThings.addAll(
            dstContext.getClusters(InfluentId.filterInfluentIds(dstEntityIds, InfluentId.CLUSTER)));
      }
      ...
      List<String> dstChildEntityIds =
          buildEntityAncestorMap(dstThings, dstEntityAncestorIndex, dstContext);
      ...
      Map<String, List<FL_Link>> aggregatedLinks =
          aggregateLinks(rawLinks, direction, srcEntityAncestorIndex, dstEntityAncestorIndex);
  ...
  private static void updateAncestorIndex(
      List<Object> immutables,
      Map<String, FL_Cluster> searchIndex,
      Map<String, String> ancestorMap) {
    for (Object immutable : immutables) {

      if (immutable instanceof FL_Entity) {
        FL_Entity entity = (FL_Entity) immutable;
        PropertyHelper clusterProp =
            EntityHelper.getFirstPropertyByTag(entity, FL_PropertyTag.CLUSTER);
        ...
        } else { // we could not find an ancestor then add the entity to the ancestor map as an
          // ancestor to itself
  ...
  public static List<String> fetchLeaves(FL_Cluster cluster, ContextRead context) {
    ...
    while (!toProcess.isEmpty()) {
      ...
      for (String subclusterId : procCluster.getSubclusters()) {
        if (BaseEntityClusterer.isImmutableCluster(
            subclusterId)) { // immutable clusters are treated like leaves
      ...
      for (FL_Link link : links.get(id)) {
        ...
        String src =
            (direction == FL_DirectionFilter.SOURCE)
                ? srcAncestorIndex.get(srcId)
                : dstAncestorIndex.get(srcId);
        String destId = link.getTarget();
        String dst =
            (direction == FL_DirectionFilter.SOURCE)
                ? dstAncestorIndex.get(destId)
                : srcAncestorIndex.get(destId);
        ...
        if (aggregateLink == null) {
          ...
          FL_Property linkCount =
              new PropertyHelper(
                  PROP_LINK_COUNT,
                  PROP_LINK_COUNT,
                  1,
                  FL_PropertyType.INTEGER,
                  Collections.singletonList(FL_PropertyTag.CONSTRUCTED));
          props.add(linkCount);
          ...
          if (minDate != null && maxDate != null) {
            ...
            FL_Property dateProp =
                new PropertyHelper(
                    PROP_DATE, PROP_DATE, minDate, maxDate, FL_PropertyType.DATE, datePropTags);
            props.add(dateProp);
          }
  ...
  private static Set<String> getDestLinkEntity(
      Map<String, List<FL_Link>> links, FL_DirectionFilter direction) {
