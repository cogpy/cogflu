...
public class EntityClusterFactory {
  ...
  protected final List<ClusterDistributionProperty> distProperties =
      new ArrayList<ClusterDistributionProperty>();
  ...
  private void initDistProperties(Properties pMgr) {
    String[] properties =
        pMgr.getString(ClustererProperties.CLUSTER_PROPERTIES, "TYPE:type-dist,GEO:location-dist")
            .split(",");
  ...
  protected String getRepresentativeLabel(List<FL_Entity> entities, List<FL_Cluster> clusters) {
    ...
    if (entities != null && !entities.isEmpty()) {
      for (FL_Entity entity : entities) {
        ...
        && !clusters
            .isEmpty()) { // otherwise, if we have clusters then pick the first label we find
      for (FL_Cluster cluster : clusters) {
        PropertyHelper labelProp =
            ClusterHelper.getFirstPropertyByTag(cluster, FL_PropertyTag.LABEL);
        if (labelProp != null) {
          return (String) labelProp.getValue();
        }
      }
  ...
  protected List<FL_Property> createDistProperties(
      Map<String, Distribution> distSummaries,
      List<FL_Entity> members,
      List<FL_Cluster> subClusters,
      int decendentCount) {
        ...
        if (isGeo) {
          ...
          double size =
              (members != null ? members.size() : 0)
                  + (subClusters != null
                      ? subClusters.size()
                      : 0); // range is the average stat value
          range = freq / size;
          freq = decendentCount;
        }
  ...
  public void updateClusterProperties(
      FL_Cluster cluster, ClusterContext context, boolean updateChildren) {
    updateClusterProperties(cluster, context.entities, context.clusters, updateChildren);
  }

  public void updateClusterProperties(
      Map<String, FL_Cluster> clusters, ClusterContext context, boolean updateChildren) {
    updateClusterProperties(clusters, context.entities, context.clusters, updateChildren);
  }

  public void updateClusterProperties(
      Map<String, FL_Cluster> clusters,
  ...
  public void updateClusterProperties(
    ...
    for (String subClusterId : cluster.getSubclusters()) {
      ...
      if ((updateChildren)
          && !InfluentId.hasIdClass(subCluster.getUid(), InfluentId.CLUSTER_SUMMARY)) {
  ...
  protected List<FL_Property> computeClusterProperties(
      List<FL_Entity> members, List<FL_Cluster> subClusters) {
    ...
    if (members != null) {
      for (FL_Entity entity : members) {
        ...
        PropertyHelper inFlow =
            EntityHelper.getFirstPropertyByTag(entity, FL_PropertyTag.INFLOWING);
        if (inFlow != null) {
          inDegree += (Integer) inFlow.getValue();
        }
        PropertyHelper outFlow =
            EntityHelper.getFirstPropertyByTag(entity, FL_PropertyTag.OUTFLOWING);
        if (outFlow != null) {
    ...
    if (subClusters != null) {
      for (FL_Cluster cluster : subClusters) {
        ...
        PropertyHelper inFlow =
            ClusterHelper.getFirstPropertyByTag(cluster, FL_PropertyTag.INFLOWING);
        if (inFlow != null) {
          inDegree += (Integer) inFlow.getValue();
        }

        PropertyHelper outFlow =
            ClusterHelper.getFirstPropertyByTag(cluster, FL_PropertyTag.OUTFLOWING);
        if (outFlow != null) {
          outDegree += (Integer) outFlow.getValue();
        }
    ...
    FL_Property p =
        new PropertyHelper(
            "count", "count", decendentCount, FL_PropertyType.INTEGER, FL_PropertyTag.STAT);
    properties.add(p);
    ...
    p = new PropertyHelper("name", "name", label, FL_PropertyType.STRING, FL_PropertyTag.TEXT);
    properties.add(p);
    ...
    p =
        new PropertyHelper(
            "confidence", "confidence", avgConfidence, FL_PropertyType.DOUBLE, FL_PropertyTag.STAT);
    properties.add(p);
  ...
  public FL_Cluster toCluster(
      String clusterId, List<FL_Entity> entities, List<FL_Cluster> subClusters) {
  ...
  public FL_Cluster toAccountOwnerSummary(
      FL_Entity owner, List<FL_Entity> accounts, List<FL_Cluster> accountClusters) {
    ...
    ownerCluster.setUid(
        InfluentId.fromNativeId(InfluentId.ACCOUNT_OWNER, namespace, rawId).getInfluentId());
    ...
    label.setRange(
        SingletonRangeHelper.from(
            (String) EntityHelper.getFirstPropertyByTag(owner, FL_PropertyTag.LABEL).getValue()));
  ...
  public static void setEntityCluster(FL_Entity entity, FL_Cluster cluster) {
    ...
    if (p != null) {
      ...
      p = new PropertyHelper("cluster", cluster.getUid(), FL_PropertyTag.CLUSTER);
  ...
  public static Collection<String> findAncestorIds(
      Collection<String> objectIds, ContextRead targetContext) {
