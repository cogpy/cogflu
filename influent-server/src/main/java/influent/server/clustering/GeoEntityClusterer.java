...
public class GeoEntityClusterer extends BaseEntityClusterer {
  ...
  private FL_GeoData getGeoValue(PropertyHelper property) {
    ...
    if (property != null && property.getRange() != null) {
      ...
      // if the geo property is missing valid lat/long try to use the latitude and longitude of the
      // country
      if (geo == null
          || geo.getLat() == null
          || geo.getLon() == null
  ...
  private DataSet createDataSet(
      Collection<FL_Entity> entities, Collection<FL_Cluster> immutableClusters) {
  ...
  @SuppressWarnings("unchecked")
  private Map<String, FL_Cluster> clusterByLatLon(
  ...
  public ClusterContext clusterEntities(
    ...
    Map<String, FL_Cluster> modifiedClusters =
        clusterByLatLon(entities, immutableClusters, clusters);
