...
public class LinkSearchResource extends ApertureServerResource {
      ...
      if (termMap.size() > 0) {
        sResponse =
            _transactionsSearcher.search(
                termMap, orderBy, startIndex, startIndex + maxResults, levelOfDetail);
      } else {
  ...
  private JSONObject buildTransactionResults(
    ...
    if (groupByKey != null && !groupByKey.equals("null")) {
      Map<String, List<FL_Link>> groupedTransactions =
          groupTransactionsBy(groupByKey, matchScores, links);
    ...
    FL_PropertyDescriptors columnHeaders =
        _transactionsSearcher.getKeyDescriptors(results, orderBy);
  ...
  private Map<String, List<FL_Link>> groupTransactionsBy(
      String groupByKey, Map<String, Double> matchScores, List<FL_Link> links) {

    Map<String, List<FL_Link>> groupedTransactions = new LinkedHashMap<String, List<FL_Link>>();

    // Check if we'll be grouping by ENTITY
    boolean groupByEntityID =
        groupByKey.equals(FL_RequiredPropertyKey.ENTITY.name())
            || groupByKey.equals(FL_RequiredPropertyKey.LINKED.name());
    ...
    for (int i = 0; i < links.size(); ++i) {
      ...
      if (groupByEntityID) {
        final String source =
            (String)
                SingletonRangeHelper.value(
                    PropertyHelper.getPropertyByKey(
                            link.getProperties(), FL_RequiredPropertyKey.FROM.name())
                        .getRange());
        final String target =
            (String)
                SingletonRangeHelper.value(
                    PropertyHelper.getPropertyByKey(
                            link.getProperties(), FL_RequiredPropertyKey.TO.name())
                        .getRange());
  ...
  private PropertyMatchBuilder processDescriptors(JSONObject propertyMatchDescriptorsMap)
    ...
    final PropertyMatchBuilder terms =
        new PropertyMatchBuilder(
            propertyMatchDescriptorsMap, true, _applicationConfiguration.hasMultipleEntityTypes());
