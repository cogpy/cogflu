...
public class LeafEntityLookupResource extends ApertureServerResource {
  ...
  private void getContainedEntitiesHelper(
      List<String> clusterIds, List<String> leafEntities, ContextRead context) {
    ...
    for (FL_Cluster result : clusterResults) {
      PropertyHelper ownerIdProp =
          ClusterHelper.getFirstPropertyByTag(result, FL_PropertyTag.ACCOUNT_OWNER);
