...
public class ModifyContextResource extends ApertureServerResource {
  ...
  private StringRepresentation createReponse(
      List<Object> modifiedRootObjects, String targetContextId, String sessionId)
  ...
  public StringRepresentation modifyContext(String jsonData) throws ResourceException {
    ...
    try {
      ...
      final String srcContextId =
          request.getString("sourceContextId", null); // source context to insert entities from
      if (srcContextId != null && !GuidValidator.validateContextString(srcContextId)) {
        throw new ResourceException(
            Status.CLIENT_ERROR_EXPECTATION_FAILED, "srcContextId is not a valid context id");
      }

      final String targetContextId =
          request.getString("targetContextId", null); // the context to modify
      if (targetContextId != null && !GuidValidator.validateContextString(targetContextId)) {
        throw new ResourceException(
            Status.CLIENT_ERROR_EXPECTATION_FAILED, "targetContextId is not a valid context id");
      }
        ...
        if (edit.equalsIgnoreCase("insert")) {
          if (targetContextId == null) {
            ...
            return createReponse(
                insert(entityIds, srcContextId, targetContextRW, sessionId),
                targetContextId,
                sessionId);
          }
          ...
          if (srcContextId == null) {
            ...
            return createReponse(
                remove(entityIds, targetContextRW, sessionId), targetContextId, sessionId);
          }
  ...
  private List<Object> insert(
    ...
    if (sourceContextId == null && cIds.size() > 0) {
      throw new IllegalArgumentException(
          "It is not valid to attempt to insert clusters or cluster summaries into a context without a valid source context id.");
    }
    ...
    // returns root level entities that have no containing cluster and all the simplified clusters
    // in the context
    Pair<Collection<FL_Entity>, Collection<FL_Cluster>> simpleContext =
        ContextCollapser.collapse(targetContext, false, null);
    ...
    // files want a single root cluster so we create one and give it an id same as the target
    // context
    if (targetContextId.startsWith("file")) {
      // we set the the root id to be the same id as the file context id

      String rootId =
          InfluentId.fromNativeId(InfluentId.CLUSTER, "cluster", targetContextId).getInfluentId();
      int version = 1;
  ...
  private List<Object> remove(
    ...
    // find the ancestors of the childIds to be removed - we return revised versions of ancestors to
    // UI to update display
    Collection<String> ancestorIds =
        EntityClusterFactory.findAncestorIds(removalIds, targetContext);
