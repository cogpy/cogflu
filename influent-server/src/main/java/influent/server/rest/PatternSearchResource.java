...
public class PatternSearchResource extends ApertureServerResource {
  ...
  protected static final int DEFAULT_MAX_LIMIT = 50;
  ...
  public StringRepresentation search(String jsonData) throws ResourceException {
    ...
    try {
      ...
      FL_PatternDescriptor example =
          (FL_PatternDescriptor) AvroUtils.decodeJSON(FL_PatternDescriptor.getClassSchema(), term);
      ...
      for (int i = 0; i < exampleEntities.size(); i++) {
        ...
        if (ids.size() == 0) hasExamplars = false;
      ...
      if (response instanceof FL_Future) {
        ...
        searchResults =
            FL_PatternSearchResults.newBuilder()
                .setTotal(0)
                .setResults(Collections.<FL_PatternSearchResult>emptyList())
                .build();
      }

      final Map<String, List<FL_EntityMatchResult>> roleResults =
          new LinkedHashMap<String, List<FL_EntityMatchResult>>();
      ...
      final Map<String, List<FL_EntityMatchResult>> entityInstances =
          new HashMap<String, List<FL_EntityMatchResult>>();
      ...
      for (FL_PatternSearchResult searchResult : searchResults.getResults()) {
        ...
        for (FL_EntityMatchResult entityResult : searchResult.getEntities()) {
          ...
          String entityId =
              InfluentId.fromNativeId(entityType, entityNamespace, nId).getInfluentId();
          ...
          if (roleMatches != null) {
            ...
            for (i = 0; i < roleMatches.size(); i++) {
              if (roleMatches.get(i).getEntity().getUid().toString().equals(entityId)) {
                roleMatches
                    .get(i)
                    .setMatchScore(
                        roleMatches.get(i).getMatchScore() + entityResult.getMatchScore());
                break;
              }
            }
            ...
            if (i == roleMatches.size()) {
              ...
              // keep index of all enriched entity instances by id so we can slot in full entity
              // details later.
      ...
      final List<FL_Entity> searchedEntities =
          dataAccess.getEntities(
              new ArrayList<String>(entityInstances.keySet()), FL_LevelOfDetail.SUMMARY);
      ...
      for (FL_Entity entity : searchedEntities) {
        ...
        if (entity.getTags().contains(FL_EntityTag.ACCOUNT_OWNER)) {
          FL_Property summary =
              EntityHelper.getFirstPropertyByTag(entity, FL_PropertyTag.CLUSTER_SUMMARY);
      ...
      for (FL_Cluster clusterSummary :
          clusterAccess.getClusterSummary(new ArrayList<String>(clusterSummaries.values()))) {
        String ownerId =
            (String)
                ClusterHelper.getFirstPropertyByTag(clusterSummary, FL_PropertyTag.ACCOUNT_OWNER)
                    .getValue();

        summaries.put(ownerId, clusterSummary);
      }

      // Ok, now process the entities.  If the entity key is in the accounts map, construct account
      // owner, otherwise use the entity
      for (FL_Entity entity : searchedEntities) {
        ...
        if (accountsForAccountOwners.containsKey(entity.getUid())) {
          ...
          if (accounts != null) {
            FL_Cluster owner =
                clusterFactory.toAccountOwnerSummary(
                    entity, accounts, new ArrayList<FL_Cluster>(0));
            owners.put(owner.getUid(), owner);
          }
        } else if (!clusterSummaries.containsKey(
            entity.getUid())) { // if not a cluster summary then add entity
      ...
      for (Map.Entry<String, List<FL_EntityMatchResult>> entry : roleResults.entrySet()) {
        ...
        for (FL_EntityMatchResult emr : entry.getValue()) {
          ...
          try {
            ...
            s_logger.error(
                "Error serializing entity with uid: " + Uid + ".   Omitting from results.");
            continue;
          }
        }
  ...
  public static void normalizeMatchScores(Map<String, Double> matchScores) {
    ...
    for (String entity : matchScores.keySet())
      if (matchScores.get(entity) > maxscore) maxscore = matchScores.get(entity);
