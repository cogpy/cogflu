...
public class RelatedLinkResource extends ApertureServerResource {
  ...
  public StringRepresentation getLinks(String jsonData) throws ResourceException {
    ...
    // This list is populated with the entities at the other end of the discovered links, and pass
    // to the UI.
    // This is so the UI doesn't need to make a second call to get the entities at the end of the
    // found links.
    ...
    try {
      ...
      if (linktype != null) {
        if (linktype.equalsIgnoreCase("source")) direction = FL_DirectionFilter.SOURCE;
      ...
      // the "entity" to fetch related links for - entity could be one of: file cluster, entity
      // cluster or raw entity
      ...
      // retrieve and aggregate the related links for the flow entities using the appropriate
      // filters
      // If no focus filter is specified this will cluster the related entities and aggregate the
      // related links accordingly
      Map<String, List<FL_Link>> relatedLinks =
          clusterAccess.getFlowAggregation(
              flowEntityIds, focusIds, direction, dateRange, contextId, targetContextId);
      ...
      try {
        final ContextReadWrite targetContextRW =
            contextCache.getReadWrite(targetContextId, permits);

        dms = System.currentTimeMillis();

        // collapse context entity cluster hierarchy by simplifying the tree for usability
        // returns root level entities that have no containing cluster and all the simplified
      ...
      // if we have targets and targets were requested by the client then serialize them into the
      // response
      ...
      // and update the source / target's if the context was simplified - they may now point to a
      // simplified object
