...
public class EntitySearchResource extends ApertureServerResource {
  ...
  private Map<String, List<Object>> groupEntitiesBy(
      String groupByKey, Map<String, Double> matchScores, List<FL_Entity> entities) {
    Map<String, List<Object>> groupedEntities = new LinkedHashMap<String, List<Object>>();

    final FL_PropertyDescriptor pd =
        PropertyDescriptorHelper.find(groupByKey, _searchDescriptors.getProperties());
  ...
  private JSONObject serializeEntity(Object entity) {
    ...
    try {
      ...
      if (entity instanceof FL_Cluster) {
        ...
        jo.put(
            "memberEntities",
            UISerializationHelper.toUIJson(
                _dataAccess.getEntities(cluster.getMembers(), FL_LevelOfDetail.FULL)));
      } else {
  ...
  public StringRepresentation search(String jsonData) throws ResourceException {
    ...
    try {
      ...
      FL_SearchResults sResponse =
          _entitySearcher.search(
              termMap, orderBy, (long) startIndex, (long) resultLimit, levelOfDetail);
      ...
      if (groupByKey != null && !groupByKey.equals("null")) {
        Map<String, List<Object>> groupedEntities =
            groupEntitiesBy(groupByKey, matchScores, entities);
  ...
  private PropertyMatchBuilder processSearchTerms(String query) {
    ...
    final PropertyMatchBuilder terms =
        new PropertyMatchBuilder(
            query, _searchDescriptors, false, _applicationConfiguration.hasMultipleEntityTypes());
  ...
  private PropertyMatchBuilder processDescriptors(JSONObject propertyMatchDescriptorsMap)
    ...
    final PropertyMatchBuilder terms =
        new PropertyMatchBuilder(
            propertyMatchDescriptorsMap, false, _applicationConfiguration.hasMultipleEntityTypes());
