...
public class RestoreStateResource extends ApertureServerResource {
  ...
  @Inject
  public RestoreStateResource(
      FL_Persistence persistence,
  ...
  public StringRepresentation restoreState(String jsonData) throws ResourceException {
    try {
      ...
      if (sessionId == null || sessionId.length() == 0) {
        throw new ResourceException(
            Status.CLIENT_ERROR_BAD_REQUEST, "No sessionId found in persistence request");
      }
      ...
      getResponse().setCacheDirectives(Collections.singletonList(CacheDirective.noCache()));
