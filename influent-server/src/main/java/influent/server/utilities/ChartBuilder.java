...
public class ChartBuilder {
  ...
  public ChartBuilder(
      FL_LinkSearch transactionsSearcher, FL_ClusteringDataAccess da, String ehCacheConfig) {
  ...
  public ChartData computeChart(
    ...
    if (!foundInCache)
      startingBalance =
          calcStartingBalance(DateTimeParser.fromFL(dateRange.getStartDate()), entities);
    ...
    if (!foundInCache) {
      Map<String, List<FL_Link>> links =
          da.getTimeSeriesAggregation(
              entities, focusEntities, dateRange, contextId, focusContextId);
          ...
          for (FL_Link financialLink : financialLinks) {
            ...
            for (FL_Property prop : financialLink.getProperties()) {
              ...
              if (property.hasTag(FL_PropertyTag.DATE)) {
                ...
                bucket =
                    DateRangeBuilder.determineInterval(
            ...
            if (!skip) {
              ...
              if ((sourceId != null && focusEntities != null && focusEntities.contains(sourceId))
                  || (targetId != null
                      && focusEntities != null
                      && focusEntities.contains(targetId))) {
                focusOverride = true;
              }
              if (financialLink.getDirected()) {
                if (sourceId.equals(entity)) {
                  focusDebits.set(
                      bucket, focusDebits.get(bucket) + (amount == null ? 0.0 : amount));
                } else {
                  focusCredits.set(
                      bucket, focusCredits.get(bucket) + (amount == null ? 0.0 : amount));
                }
              } else {
                if (focusOverride) {
                  if (sourceId != null) {
    ...
    if (bDebugChartData) {
      ...
      System.out.print("\nFC:");
      for (Double credit : focusCredits) {
        System.out.print(credit + ",");
      }
      System.out.print("\nFD:");
      for (Double debit : focusDebits) {
        System.out.print(debit + ",");
      }
      System.out.println("\n##############################################");
    }
    ...
    return new ChartData(
        ...
        focusCredits,
        focusDebits,
        DataAccessHelper.getDateIntervals(dateRange),
        hash.getHash());
  }
  ...
  private Double calcStartingBalance(DateTime minDate, List<String> memberIds) {
    ...
    //		Map<String, List<Link>> links = null;// da.getRelatedLinks(memberIds, EntityLinkType.BOTH,
    // filters, AggregationType.TIME_SERIES, null,null);
    //		if (links != null && links.size() > 0) {
