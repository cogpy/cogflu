...
public class DownloadExchangeRates {
  ...
  private static final String QUERY_TEMPLATE =
      "http://www.oanda.com/currency/historical-rates/download?"
   ...
   *
   * @param startDate The first date to fetch data for.
   * @param endDate The latest date to fetch data for.
   * @param currencies The list of currencies to convert, where the first currency is the base
   *     currency.
   * @param filename The name of the file to download to.
   * @param toBaseCurrency Convert to the base currency, or false from.
   * @param verbose Verbose output.
   * @param pauseInMs the amount of time in milliseconds to pause between requests, to avoid denial
   *     of service problems.
                ...
                for (int j = 2; j < fields.length; j++) {
                  ...
                  if (toBaseCurrency) {
                    try {
                      ...
                      System.err.println(
                          "Failed to parse value (run in verbose mode for location details): "
                              + line);
                      throw e;
                    }
                  }
   ...
   * Downloads
   *
   * <p>forms a request like this:
   * "http://www.oanda.com/currency/historical-rates/download?quote_currency=USD&end_date=2013-6-17&start_date=2013-5-19&period=daily&display=absolute&rate=0&data_range=d30&price=bid&view=graph&base_currency_0=DZD&base_currency_1=AFN&base_currency_2=&base_currency_3=&base_currency_4=&download=csv"
   */
  public static void main(String[] args) {
    ...
    if (currencies.size() < 2) {
      System.err.println(
          "Must specify at least two currencies: primary currency to convert to, and one currency to convert.");
      System.exit(-1);
    }
