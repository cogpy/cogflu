...
public class UISerializationHelper {
   ...
   *
   * @param list the list from the UI, typically a wrapper over a json object.
   * @param defaultOrderBy the default ordering or null if the order by list should be null when
   *     unspecified.
   * @return the list of order by instructions or null if unspecified and no default.
   */
    ...
    if (list != null) {
      for (Properties orderByObj : list) {
        ...
        if (orderKey != null && !orderKey.isEmpty()) {
          ...
          orderBy.add(
              FL_OrderBy.newBuilder().setPropertyKey(orderKey).setAscending(orderAsc).build());
        }
      }
    }
  ...
  /**
   * Helper function to convert a map of objects into a proper {@link JSONObject}. This helps to
   * make sure the mapped objects are properly turned into JSONObjects as well.
   *
   * @param map
   * @return
  ...
  public static JSONObject toUIJson(Map<?, ?> map) throws JSONException {
    ...
    for (Entry<?, ?> entry : map.entrySet()) {
      ...
      if (o instanceof Map) {
        ...
        try {
          ret.put(
              key, new JSONObject(SerializationHelper.toJson(o, ((SpecificRecord) o).getSchema())));
        } catch (IOException e) {
  ...
  /**
   * Helper function to convert a list of objects into a proper {@link JSONObject}. This helps to
   * make sure the list contents are properly turned into JSONObjects as well.
   *
   * @param list
   * @return
   * @throws JSONException
  ...
  public static JSONObject toUIJson(FL_PropertyDescriptor flpd) throws JSONException {
    ...
    final String searchableBy =
        flpd.getSearchableBy() != null
            ? flpd.getSearchableBy().name()
            : FL_SearchableBy.DESCRIPTOR.name();
  ...
  public static List<String> buildListFromJson(JSONObject jsonObject, String listName)
      throws JSONException {
