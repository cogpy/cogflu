...
public class GraphMLImportDataService implements ImportDataService {
  ...
  @Override
  public JSONObject importFromXML(String xmlData)
   ...
   * JSONObject members and JSONArray members of length 1, and omits empty JSONArrays entirely. The
   * client needs the types/contents to be consistent, so we need to replace certain JSONObjects
   * with JSONArrays after unmarshalling. All this function does is place/replace the fieldKey
   * member of rootObj with a JSONArray.
   *
   * @param rootObj
   * @param fieldKey
