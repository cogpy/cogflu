...
/**
 * Demonstration example showing OpenCog integration with Influent for cognitive 
 * transaction flow analysis.
 * 
 * This example illustrates how OpenCog's AtomSpace and reasoning capabilities
 * can be used to analyze financial transaction patterns, detect anomalies,
 * and generate cognitive insights.
 */
public class CognitiveTransactionAnalysisDemo {

    public static void main(String[] args) {
        System.out.println("=== OpenCog Integration with Influent: Cognitive Transaction Analysis Demo ===\n");
        ...
        try {
            ...
            TransactionEntity bank1 = new TransactionEntity(
                "BANK_001", "financial_institution", "commercial_bank", 
                LocalDateTime.now().minusYears(10), "active", null, "New York", 
                1000, 50000000.0, 50000.0
            );
            
            TransactionEntity bank2 = new TransactionEntity(
                "BANK_002", "financial_institution", "investment_bank", 
                LocalDateTime.now().minusYears(8), "active", null, "London", 
                800, 75000000.0, 93750.0
            );
            
            TransactionEntity account1 = new TransactionEntity(
                "ACC_12345", "customer_account", "checking", 
                LocalDateTime.now().minusYears(2), "active", 25000.0, "California", 
                150, 500000.0, 3333.33
            );
            
            TransactionEntity account2 = new TransactionEntity(
                "ACC_67890", "customer_account", "savings", 
                LocalDateTime.now().minusYears(1), "active", 75000.0, "Texas", 
                75, 200000.0, 2666.67
            );
            
            TransactionEntity suspicious = new TransactionEntity(
                "ACC_99999", "customer_account", "offshore", 
                LocalDateTime.now().minusMonths(1), "flagged", 1000000.0, "Cayman Islands", 
                10, 5000000.0, 500000.0
            );
            
            List<TransactionEntity> entities = Arrays.asList(bank1, bank2, account1, account2, suspicious);
            ...
            List<Transaction> transactions = Arrays.asList(
                // Normal transactions
                new Transaction("TXN_001", "ACC_12345", "ACC_67890", 1000.0, 
                               LocalDateTime.now().minusDays(5), "Transfer", "personal"),
                new Transaction("TXN_002", "ACC_67890", "BANK_001", 500.0, 
                               LocalDateTime.now().minusDays(4), "Payment", "fee"),
                               
                // Suspicious patterns - large amounts to offshore account
                new Transaction("TXN_003", "ACC_12345", "ACC_99999", 100000.0, 
                               LocalDateTime.now().minusDays(3), "Transfer", "investment"),
                new Transaction("TXN_004", "ACC_67890", "ACC_99999", 150000.0, 
                               LocalDateTime.now().minusDays(2), "Transfer", "investment"),
                               
                // Circular transaction pattern (potential money laundering)
                new Transaction("TXN_005", "ACC_99999", "BANK_002", 200000.0, 
                               LocalDateTime.now().minusDays(1), "Wire Transfer", "business"),
                new Transaction("TXN_006", "BANK_002", "ACC_12345", 180000.0, 
                               LocalDateTime.now(), "Wire Transfer", "loan"),
                               
                // Rapid sequence transactions (structuring pattern)
                new Transaction("TXN_007", "ACC_12345", "ACC_99999", 9500.0, 
                               LocalDateTime.now().minusHours(2), "Transfer", "gift"),
                new Transaction("TXN_008", "ACC_12345", "ACC_99999", 9800.0, 
                               LocalDateTime.now().minusHours(1), "Transfer", "gift"),
                new Transaction("TXN_009", "ACC_12345", "ACC_99999", 9700.0, 
                               LocalDateTime.now().minusMinutes(30), "Transfer", "gift")
            );
    ...
    private static void displayResults(AnalysisResult result) {
        ...
        if (patterns.isEmpty()) {
            ...
            for (CognitivePattern pattern : patterns) {
                System.out.printf("   Pattern: %s (%.2f confidence)\n", 
        ...
        if (anomalies.isEmpty()) {
            ...
            for (AnomalyDetection anomaly : anomalies) {
                System.out.printf("   Anomaly: %s (Score: %.2f)\n", 
